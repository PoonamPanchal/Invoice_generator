...
def add_invoice(request):
    if request.method=="POST":
        ...
        for i in range(0,len(total_items)):
            invoice_details['items'].append({'name':request.POST.getlist('name')[i],'quantity':int(request.POST.getlist('quantity')[i]),'price':float(request.POST.getlist('price')[i]),'amount':float(request.POST.getlist('amount')[i]),'tax':float(request.POST.getlist('tax')[i]),'total_amount':float(request.POST.getlist('total')[i])})
        insert_data=Invoices(invoice_id=invoice_id,details=json.dumps(invoice_details,indent=4))
        insert_data.save()
